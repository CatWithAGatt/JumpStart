# Write a function that given an array, returns another array with each of the
...
# A Slippery Number is a number that has 3 as a factor or has 5 as a factor,
# but does *not* have both as factors. For example, 6 is a Slippery Number,
# but 30 is not. Write a function that given an N, returns an array of the
...
def is_slippery?(number)
  (number % 3 == 0 && !(number % 5 == 0)) || (!(number % 3 == 0) && number % 5 == 0)
end
...
# You'll want to write a helper function that checks whether a given number
# is a magic number.
# Reminder: you can convert an integer to a string using #to_s. You can convert
...
def is_magic_number?(number)
  number.to_s.split('').inject(0) { |sum, x| sum + x.to_i } == 7
end
